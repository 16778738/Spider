...
class DB:
    ...
    # 存IP
    def save(self, ip):
        sql = "insert into ip_pool_http values(%s)"
    ...
    # 取所有IP
    def get_all_ip(self):
        sql = "select * from ip_pool_http"
        self.cursor.execute(sql)
        return self.cursor.fetchall()

    # 删IP
    def delete_ip(self, ip):
        sql = 'delete from ip_pool_http where ip=%s'
        self.cursor.execute(sql, [ip])
        self.conn.commit()
...
class Pool:
    ...
    def check_ip(self, ip):
        try:
            target = requests.get("http://httpbin.org/ip", proxies=ip).text
